...
public class MovieCreateDto {
    @NotBlank
    @Size(max = 255)
    @Schema(description = "The description of the movie", example = "Inception", requiredMode = Schema.RequiredMode.REQUIRED)
    private String title;

    @NotNull(message = "Release date is required")
    @Min(1888)
    @Max(2052)
    @Schema(description = "Year of release", example = "2010", requiredMode = Schema.RequiredMode.REQUIRED)
    private Integer releaseDate;

    @NotNull(message = "Producer ID is required")
    @Schema(description = "ID producer", example = "1", requiredMode = Schema.RequiredMode.REQUIRED)
